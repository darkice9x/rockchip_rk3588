...
install_cinnamon() {
	...
	dconf write /org/cinnamon/desktop/interface/gtk-theme "'Arc-Dark'"
	dconf write /org/cinnamon/desktop/wm/preferences/theme "'Arc-Dark'"
	dconf write /org/cinnamon/theme/name "'Arc-Dark'"
	dconf write /org/cinnamon/desktop/interface/icon-theme "'Arc-Dark'"
...
case "$CHOICE" in
  ...
  3)
    ...
    install_base
    install_lightdm
    sudo apt install -y lxqt lxqt-core lxqt-config lxqt-panel openbox
    ;;
  4)
    echo "[*] Installing GNOME..."
    install_base
    install_gdm
    sudo apt install -y gnome-session gnome-shell gnome-terminal nautilus ubuntu-desktop
    ;;
  5)
    echo "[*] Installing KDE Plasma..."
    install_base
    install_sddm
    sudo apt install -y kde-plasma-desktop plasma-nm dolphin konsole
    ;;
  6)
    echo "[*] Installing MATE..."
    install_base
    install_lightdm
    sudo apt install -y mate-desktop-environment mate-terminal caja
    ;;
  7)
    echo "[*] Installing minimal X11 only..."
    install_base
    sudo echo "No desktop environment installed."
    ;;
  0)
    echo "Exit."
    exit 0
